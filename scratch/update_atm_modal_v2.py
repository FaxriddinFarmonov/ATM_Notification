          ...
          <div class="flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scroll">
            ...
            <div v-else-if="detail" class="space-y-6">
              ...
              <div v-if="monthlyStats.length > 0" class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3">
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-800 max-h-96 custom-scroll">
                  <table class="w-full text-xs text-left">
...
async function exportPdf() {
  ...
  try {
    ...
    // 1. Create off-screen clone with full scroll height & unclipped tables
    ...
    // Remove scrollbars & height limits from scroll containers in clone
    const scrollContainers = clone.querySelectorAll('.overflow-y-auto, .max-h-80, .max-h-96, .max-h-\\[92vh\\]');
    ...
    // Append additional pages for full tables
    while (heightLeft > 0) {
...
print("AtmDetailModal.vue v2 updated successfully!")
