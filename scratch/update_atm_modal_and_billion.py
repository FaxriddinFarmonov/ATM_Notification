...
# 1. Update AtmDetailModal.vue
atm_modal_code = '''<template>
  <Teleport to="body">
    <Transition name="atm-modal">
      ...
      >
        ...
        >
          <!-- Modal Header -->
          <div class="flex items-center justify-between gap-4 px-6 py-5 border-b border-slate-800 bg-slate-950/60">
            ...
            <div class="flex items-center gap-3">
              <button
                ...
                class="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 via-rose-600 to-pink-600 hover:from-red-500 hover:to-pink-500 text-white font-extrabold text-xs shadow-lg shadow-red-500/25 border border-red-400/40 transition-all active:scale-95 disabled:opacity-50"
              >
          ...
          <div class="flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scroll">
            ...
            <div v-else-if="detail" class="space-y-6">
              <!-- Technical & General Info Grid Cards -->
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3">
                  <div class="flex items-center gap-2 text-xs font-bold text-emerald-400 uppercase tracking-wider">
                    ...
                    <span>Servis Shartnoma</span>
                  </div>
                  <div class="grid grid-cols-2 gap-3 pt-1">
                    <div class="p-3 bg-slate-900 rounded-xl border border-slate-800">
                      <p class="text-[11px] text-slate-400 font-bold">BTech oylik</p>
                      <p class="text-sm font-black text-emerald-400 tabular-nums mt-0.5">
                        {{ formatUzSum(detail.service_contract?.btech_monthly_fee) }}
                      </p>
                    </div>
                    <div class="p-3 bg-slate-900 rounded-xl border border-slate-800">
                      <p class="text-[11px] text-slate-400 font-bold">Glob oylik</p>
                      <p class="text-sm font-black text-sky-400 tabular-nums mt-0.5">
              ...
              <div v-if="monthlyStats.length > 0" class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3">
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-800 max-h-80 custom-scroll">
                  <table class="w-full text-xs text-left">
...
async function exportPdf() {
  ...
  try {
    const element = document.getElementById('atm-detail-pdf-report');
    if (!element) return;
...
with open(modal_path, 'w', encoding='utf-8') as f:
    f.write(atm_modal_code)

print("AtmDetailModal.vue updated successfully!")

# 2. Update FilialAylanmasiView.vue for billion formatting
view_path = r'C:\Users\Faxriddin\Documents\Frontend-ATM-Informations\src\views\FilialAylanmasiView.vue'
with open(view_path, 'r', encoding='utf-8') as f:
    view_code = f.read()

view_code = view_code.replace(
    "label: 'Daromad (mln UZS)',",
    "label: 'Daromad (Mlrd UZS)',"
)
view_code = view_code.replace(
    "data: filteredRegions.value.map((r) => Math.round((r.total_income || 0) / 1000000)),",
    "data: filteredRegions.value.map((r) => +((r.total_income || 0) / 1_000_000_000).toFixed(2)),"
)
view_code = view_code.replace(
    'y-axis-unit="mln UZS"',
    'y-axis-unit="Mlrd UZS"'
)

with open(view_path, 'w', encoding='utf-8') as f:
    f.write(view_code)

print("FilialAylanmasiView.vue billion unit updated successfully!")
